...
➕ Add Tasks:
• add "Task Name" at 7:00 am
• add "Gym" at 7:00 am every Monday
• add "Meeting" at 2:00 pm special
• add "Special event" on first Sunday of every month special
...
💡 Tips:
• Use "special" for important tasks (reminder 1 day before)
• Recurring tasks: every Monday, every 2 weeks, first Sunday of month
• Times: 7:00 am, 2:30 pm, etc.
...
Type any command to get started!`;
  ...
  async handleDeleteCommand(userId, message) {
    try {
      ...
      if (identifier.type === 'id') {
        await this.database.deleteTask(identifier.value, userId);
  ...
  async handleUpdateCommand(userId, message) {
    try {
      ...
      let task;
      if (updateData.identifier.type === 'id') {
        task = await this.database.getTaskById(updateData.identifier.value, userId);
      } else {
        // Find task by title
        const tasks = await this.database.getTasks(userId);
  ...
  async handleDeleteCommandReply(userId, replyToken, message) {
    try {
      ...
      if (identifier.type === 'id') {
        await this.database.deleteTask(identifier.value, userId);
        await this.sendReplyMessage(replyToken, "✅ Task deleted successfully!");
      } else {
